...
echo"the subject is {$sub}";
        ...
        $values[$sub]['sgrade']=$sgrade;
        echo"is the number{$sgrade} \n";
        
        $agrade=strip_tags($cells[0][11]);
          $values[$sub]['agrade']=$agrade;
             echo"is the number{$agrade} \n";
             $bgrade=strip_tags($cells[0][12]);
               $values[$sub]['bgrade']=$bgrade;
              echo"is the number{$bgrade} \n";
              $cgrade=strip_tags($cells[0][13]);
                $values[$sub]['cgrade']=$cgrade;
              echo $cgrade;
              $dgrade=strip_tags($cells[0][14]);
                $values[$sub]['dgrade']=$dgrade;
               echo "the d grade is{$dgrade}";
              $egrade=strip_tags($cells[0][15]);
                $values[$sub]['egrade']=$egrade;
                echo "the e grade is{$egrade}";
              $fgrade=strip_tags($cells[0][16]);
                $values[$sub]['fgrade']=$fgrade;
                 echo "the f grade is{$fgrade}";
              $marks=strip_tags($cells[0][17]);
                $values[$sub]['marks']=$marks;
                 echo "the marks is{$marks}";
              $grade=strip_tags($cells[0][18]);
                $values[$sub]['grade']=$grade;
...
print(stripslashes(json_encode($table)));
